class MusicLibraryController
  ...
  def list_artists
    Artist.all.uniq.sort_by(&:name).each.with_index(1) do |art, idx|
      puts "#{idx}. #{art.name}"
    end
  end
  
  def list_genres
    Genre.all.uniq.sort_by(&:name).each.with_index(1) do |gn, idx|
      puts "#{idx}. #{gn.name}"
    end
  end
  ...
  def list_songs_by_artist
    ...
    if artist = Artist.find_by_name(input) 
      artist.songs.uniq.sort_by(&:name).each.with_index(1) do |song, idx|
  ...
  def play_song
    ...
    if (1..Song.all.length).include?(input)
      song = Song.all.uniq.sort_by(&:name)[input - 1]
    end
