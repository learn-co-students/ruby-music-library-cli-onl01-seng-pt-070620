class MusicLibraryController
  ...
  def list_songs_by_artist
    ...
    input = gets.strip
      if artist_selection = Artist.find_by_name(input)
        artist_selection.songs.sort_by{|song| song.name }.each.with_index(1) do |song, index|
  ...
  def list_songs_by_genre
    ...
    input = gets.strip
      if genre_selection = Genre.find_by_name(input)
        genre_selection.songs.sort_by{|song| song.name }.each.with_index(1) do |song, index|
