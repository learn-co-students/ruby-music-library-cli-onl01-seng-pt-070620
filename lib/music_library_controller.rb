...
class MusicLibraryController 
    ...
    def play_song 
        ...
        input = gets.strip.to_i # call to_i to get an actual integer to work with 
        if (1..Song.all.length).include?(input)
            arr = Song.all.sort {|a,b| a.name <=> b.name}
            # define var that represents actual song to be played set queal to arr variable at index of input
            s = arr[input-1]
        end 
        #binding.pry
        if s 
